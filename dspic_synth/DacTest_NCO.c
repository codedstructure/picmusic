...
fractional DacLBufferA[NUMSAMP] __attribute__((space(dma)));	// Ping-pong buffer A
fractional DacLBufferB[NUMSAMP] __attribute__((space(dma)));	// Ping-pong buffer B
fractional DacRBufferA[NUMSAMP] __attribute__((space(dma)));	// Ping-pong buffer A
fractional DacRBufferB[NUMSAMP] __attribute__((space(dma)));	// Ping-pong buffer B
...
/* 	for (i=0; i<NUMSAMP; i++)
 	{
     	DacLBufferA[i] =DacLBufferB[i] = i<NUMSAMP/2?0x7fff:0;
     	DacRBufferA[i] =DacRBufferB[i] = i<NUMSAMP/2?0x7fff:0;
    }
*/
    O[0].osc = 0;
    O[0].incr = noteincr[70];
    O[1].osc = 0;
    O[1].incr = noteincr[75];
    O[2].osc = 0;
    O[2].incr = noteincr[82];
    ...
    while (true)
    {
        if ( sample < NUMSAMP )
        {
            ...
            tempL = sintab[HI_WORD_OF_32(O[0].osc) >> 6];
            tempR = sintab[HI_WORD_OF_32(O[1].osc) >> 6];
            //temp += sintab[(HI_WORD_OF_32(O[2].osc)/5) >> 6];
            tempL >>= 1;
            tempR >>= 1;
/*            switch (waveform)
            {
