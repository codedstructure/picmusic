...
enum WaveType waveform = WAVE_SINE;
...
*/
    ...
    {
        // program change
        if (rxmsg[1] <= WAVE_END) waveform = rxmsg[1];
    }
    else if ( rstat == 0xE0 )
    {
        // pitch bend
...
void cycle(void)
{
    if ( sample < NUMSAMP )
    {
        ...
        */
        ...
        unsigned short value=(HI_WORD_OF_32(O[0].phase) >> 6);
        // value should be in range 0..1023

        switch (waveform)
        {
            case WAVE_SINE:
                tempL = tempR = sintab[value];
                break;

            case WAVE_SAWTOOTH:
                tempL = tempR = ((short)(value - 512)) * 63;
                break;

            case WAVE_PULSE:
                // value >> 2 = 0..255
                // modvalue = 0..127
                // only go up to 50:50, no point going from
                // 1::127 to 127::1, as will send same at
                // both sides.
                tempL = tempR = ((value>>2) > modvalue) ? 32000 : -32000;
                break;
    ...
    // else output buffer is full - happy days.
